...
const TripleCombo = () => {
    ...
    const gunA_ComboList = [{ id: '1a', name: 'Combo 1a' }, { id: '2a', name: 'Combo 2a' }, { id: '3a', name: 'Combo 3a' }];
    const gunB_ComboList = [{ id: '1b', name: 'Combo 1b' }, { id: '2b', name: 'Combo 2b' }, { id: '3b', name: 'Combo 3b' }];
    ...
    const styles = {
        container: { padding: "1rem 2rem", fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif", backgroundColor: "#f8fafc", minHeight: "calc(100vh - 80px)", maxHeight: "calc(100vh - 80px)", overflowY: "auto" },
        flashMessage: { textAlign: "center", color: "#dc2626", fontSize: "1.2rem", fontWeight: "600", padding: "0.75rem", marginBottom: "1rem", background: "linear-gradient(135deg, #fef2f2 0%, #fee2e2 100%)", borderRadius: "8px", border: "2px solid #fecaca", animation: "flash 2s infinite" },
        header: { display: "flex", justifyContent: "flex-end", alignItems: "center", marginBottom: "1rem" },
        modeLabel: { fontSize: "1.1rem", fontWeight: "600", color: themeColor, background: `linear-gradient(135deg, rgba(${themeColorRgb}, 0.1) 0%, rgba(${themeColorRgb}, 0.2) 100%)`, padding: "0.5rem 1.5rem", borderRadius: "20px", border: `2px solid ${themeColor}` },
        gunContainer: { display: "flex", gap: "1.5rem", height: "calc(100vh - 200px)" },
        gunPanel: { flex: 1, background: "#ffffff", border: `2px solid ${themeColor}`, display: "flex", flexDirection: "column", overflow: "hidden" },
        ...
        sectionHeader: { fontSize: "0.95rem", fontWeight: "700", color: "#ffffff", textTransform: "uppercase", letterSpacing: "1px", padding: "0.6rem 1rem", background: `rgba(${themeColorRgb}, 0.7)`, borderBottom: `2px solid ${themeColor}` },
        tableRow: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.75rem 1rem", borderBottom: `1px solid ${themeColor}`, backgroundColor: "#ffffff" },
        tableRowAlt: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.75rem 1rem", borderBottom: `1px solid ${themeColor}`, backgroundColor: "#f8fafc" },
        toggleLabel: { fontSize: "1rem", fontWeight: "500", color: "#334155" },
        ...
        toggleCircle: { width: "22px", height: "22px", borderRadius: "50%", backgroundColor: "#ffffff", position: "absolute", top: "3px", boxShadow: "0 2px 4px rgba(0,0,0,0.2)" },
    };

    const getToggleStyle = (isActive) => ({ ...styles.toggleButton, backgroundColor: isActive ? themeColor : "#cbd5e1" });
    const getToggleCircleStyle = (isActive) => ({ ...styles.toggleCircle, left: isActive ? "27px" : "3px" });
    const getStateStyle = (isActive, type) => ({ ...styles.toggleState, color: type === "on" ? (isActive ? themeColor : "#94a3b8") : (!isActive ? "#64748b" : "#94a3b8") });
    ...
    return (
        <div style={styles.container}>
            ...
            <div style={styles.header}><span style={styles.modeLabel}>Selected Mode: Triple Combo</span></div>
